...
import { PersonalizeConfig } from '@/types/generic'

const personalizeConfiguration: PersonalizeConfig = {
    uid: '',
    audiences: {},
    taxonomy_path: ''
}
...
export const PersonalizationProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [personalizeConfig, setPersonalizeConfig] = useState<PersonalizeConfig | undefined>()
    ...
    const getPersonalizationConfigFromCMS = async () => {
        try{
            const personalize_config = await getEntries(
                {
                    modelDefintion: { contentTypeUid: 'personalize_config' },
                    locale: process.env.NEXT_PUBLIC_DEFAULT_LOCALE ?? 'en-us',
                    limit: 1,
                }, personalizationSDK) as PersonalizeConfig[]
